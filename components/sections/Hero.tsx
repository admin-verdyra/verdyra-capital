...
export default function Hero() {
  ...
  return (
    <section
      id="home"
      className="relative overflow-hidden bg-[#F8FAF9] px-6 pb-10 pt-20 sm:px-8 lg:px-10 lg:pb-12 lg:pt-24"
    >
        ...
        <div className="relative z-10 max-w-2xl">
          ...
          <div className="mt-6 rounded-[32px] bg-gradient-to-r from-[#0F5A3A] to-[#146A47] p-6 text-white shadow-[0_24px_70px_rgba(15,90,58,0.20)]">
            ...
            <div className="mt-4 flex items-center gap-3">
              <span className="text-3xl font-bold sm:text-5xl">₹1L</span>

              <span className="text-2xl text-[#F2D58E]">→</span>

              <span className="text-3xl font-bold sm:text-5xl">₹25Cr</span>
            </div>
          ...
          <p className="mt-5 max-w-2xl text-base leading-8 text-slate-600 sm:text-lg">
            Helping Indian businesses secure tailored financing solutions
            through leading Banks and NBFCs with dedicated capital advisory.
          </p>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            {benefits.map((benefit) => (
              <div
                key={benefit}
                className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white/80 px-4 py-3 text-sm font-medium text-slate-800 shadow-sm"
              >
          ...
          <div className="mt-6 grid gap-3 rounded-[24px] border border-[#0F5A3A]/10 bg-white/80 p-4 shadow-[0_16px_48px_rgba(15,90,58,0.06)] sm:grid-cols-2 lg:grid-cols-4">
            {trustPoints.map((point) => (
              <div
                key={point}
                className="rounded-2xl bg-[#F8FAF9] px-3 py-3 text-center text-sm font-medium text-[#0F5A3A]"
              >
                {point}
              </div>
            ))}
          ...
          <p className="mt-4 text-xs leading-6 text-slate-500">
            *Interest rates are indicative and subject to the applicant&apos;s
            credit profile, business eligibility, lender approval and prevailing
            market conditions.
          </p>
